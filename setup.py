...
compile_args += (
    "-Wno-cpp -Wno-unused-result -Wno-unused-function -Wno-format-overflow".split()
)
compile_args += hdf5_cflags + gsl_cflags + ["-I" + np.get_include()]
link_args = (
    "-lm -fopenmp".split() + hdf5_libs + gsl_libs + ["-Wl,-rpath=" + gsl_libs[0][2:]]
)

ext_modules = [
    Extension(
        name="dragonfly.detector",
        sources=["dragonfly/detector.pyx"],
    ...
    Extension(
        name="dragonfly.emcfile",
        sources=["dragonfly/emcfile.pyx", "dragonfly/src/emcfile.c"],
    ...
    Extension(
        name="dragonfly.model",
        sources=["dragonfly/model.pyx", "dragonfly/src/model.c"],
    ...
    Extension(
        name="dragonfly.quaternion",
        sources=["dragonfly/quaternion.pyx", "dragonfly/src/quaternion.c"],
    ...
    Extension(
        name="dragonfly.iterate",
        sources=["dragonfly/iterate.pyx", "dragonfly/src/iterate.c"],
    ...
    Extension(
        name="dragonfly.params",
        sources=["dragonfly/params.pyx"],
    ...
    Extension(
        name="dragonfly.recon",
        sources=[
            "dragonfly/recon.pyx",
            "dragonfly/src/maximize.c",
            "dragonfly/src/model.c",
        ],
        depends=["dragonfly/src/maximize.h", "dragonfly/recon.pxd"],
        language="c",
        extra_compile_args=compile_args + mpi_cflags,
        extra_link_args=link_args + mpi_libs,
    ),
]
py_packages = [
    "dragonfly",
